...
{
    QFileDialog* asdf = new QFileDialog();
    ui->label->setText(asdf->getExistingDirectory(this, tr("Open Dir"), "./", QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks));
}

void MainWindow::on_dirtestButton_clicked()
{
    QByteArray array1 = ui->urlLabel->text().toLocal8Bit();
    char* buffer1 = array1.data();
    QByteArray array2 = ui->urlLabel->text().toLocal8Bit();
    char* buffer2 = array2.data();
    ui->dirtestNum->setValue(predictions(buffer1, buffer2));
}
...
void MainWindow::on_button_predict_clicked()
{
    ...
    QByteArray array1 = dirtext.toLocal8Bit();
    char* buffer1 = array1.data();
    QString dirtext2 = ui->label->text() + "/";
    qDebug() << dirtext2;
    QByteArray array2 = dirtext2.toLocal8Bit();
    char* buffer2 = array2.data();
    ui->dirtestNum->setValue(predictions(buffer1, buffer2));
    //QString d3 = dirtext + d2;
    ui->dirList->clear();
    QListWidgetItem* Pos = new QListWidgetItem(dp, ui->dirList);
    Pos->setData(Qt::UserRole, dirtext2 + dp);
    //Pos->setText(dp);
    //ui->dirList->addItem(Pos);
    QListWidgetItem* Neg = new QListWidgetItem(dn, ui->dirList);
    Neg->setData(Qt::UserRole, dirtext2 + dn);
    //Pos->setText(dn);
